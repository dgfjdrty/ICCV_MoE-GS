...
let defaultOptions = {
  ...
  let Dics = function(options) {
    ...
    if (this.container == null) {
      ...
      // 컨테이너 내에 video 태그가 있으면 video들을, 없으면 img 태그들을 선택합니다.
      if (this.container.querySelectorAll("video").length > 0) {
        this.media = this.getVideos();
      } else {
        this.media = this._getImages();
      }
      this.sliders = [];
      this._activeSlider = null;
  
  
      this._load(this.media[0]);
  
    }
  };
  
  /* === 새로 추가: 영상 선택 메서드 === */
  Dics.prototype.getVideos = function() {
   ...
   * @private
   */
  /* 수정된 _load 메서드: 이미지와 영상 모두 처리 */
  Dics.prototype._load = function(firstMedia, maxCounter = 100000) {
    // 이미지 처리
    if (firstMedia.tagName.toLowerCase() === "img") {
    ...
    // 영상 처리: loadedmetadata 이벤트를 통해 로딩 완료 확인
    else if (firstMedia.tagName.toLowerCase() === "video") {
      if (firstMedia.readyState >= 2 && firstMedia.videoWidth) {
        this._buidAfterFirstMediaLoad(firstMedia);
        window.addEventListener("resize", () => {
          this._setContainerWidth(firstMedia);
          this._resetSizes();
  ...
  /* 수정된 _calcContainerHeight: 이미지와 영상 모두 지원 */
  ...
  Dics.prototype._resetSizes = function() {
    let dics = this;
    let mediaLength = dics.media.length;
    ...
    for (let i = 0; i < sections$$.length; i++) {
      ...
      section$$.querySelector(".b-dics__media").style[this.config.positionField] = `${i * -initialMediaContainerWidth}px`;
  
      const slider$$ = section$$.querySelector(".b-dics__slider");
      if (slider$$) {
        slider$$.style[this.config.positionField] = `${initialMediaContainerWidth * (i + 1)}px`;
  ...
  Dics.prototype._build = function() {
    ...
    for (let i = 0; i < mediaLength; i++) {
      ...
      dics._createAltText(mediaElem, i, mediaContainer);
  
      dics._applyFilter(mediaElem, i, dics.options.filters);
      dics._rotate(mediaElem, mediaContainer);
  
  
      section.setAttribute("data-function", "b-dics__section");
      section.style.flex = `0 0 ${initialMediaContainerWidth}px`;
  
      mediaElem.classList.add("b-dics__media");
      ...
      mediaElem.style[this.config.positionField] = `${i * -initialMediaContainerWidth}px`;
  ...
  Dics.prototype._setEvents = function() {
    ...
    let listener = function(event) {
      ...
      let beforeSectionsWidth = dics._beforeSectionsWidth(dics.sections, dics.media, dics._activeSlider);
   ...
   * @private
   */
  Dics.prototype._setLeftToMedia = function(sections, media) {
    let size = 0;
    for (let i = 0; i < media.length; i++) {
      let mediaElem = media[i];
  
      mediaElem.style[this.config.positionField] = `-${size}px`;
  ...
  Dics.prototype._disableMediaDrag = function() {
    for (let i = 0; i < this.media.length; i++) {
      ...
      this.media[i].addEventListener("dragstart", function(e) {
        e.preventDefault();
      });
    }
  };
   ...
   * @private
   */
  Dics.prototype._createAltText = function(mediaElem, i, mediaContainer) {
    let textContent = mediaElem.getAttribute("alt") || mediaElem.getAttribute("data-alt");
    if (textContent) {
  ...
  Dics.prototype._rotate = function(image, imageContainer) {
    image.style.transform = `rotate(-${this.options.rotate})`;
    imageContainer.style.transform = `rotate(${this.options.rotate})`;
  ...
  Dics.prototype._removeActiveElements = function() {
    let self = this; // 인스턴스 참조 저장
    let activeElements = self.container.querySelectorAll(".b-dics__slider--active");
  
    for (let activeElement of activeElements) {
      activeElement.classList.remove("b-dics__slider--active");
      utils.removeMultiEvents(self.container, ["mousemove", "touchmove"], self._removeActiveElements);
    }
  };
  ...
  Dics.prototype._pushSections = function(calcMovePixels, position) {
    ...
    this._setLeftToMedia(this.sections, this.media);
